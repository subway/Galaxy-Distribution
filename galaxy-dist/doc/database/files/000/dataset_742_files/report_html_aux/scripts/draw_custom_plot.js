...
function drawScatterPlot() {
    ...
    var data = new google.visualization.DataTable();
    var rowData = [null];
    for(var i=0; i<json.assembliesNames.length; i++){
       data.addColumn('number', json.assembliesNames[i]);
	rowData.push(null);
    }
	...
	for(var y=1; y<json.report[x].length; y++){
		...
		if((ddl2Val != null) && (ddl3Val != null)){
		    alert(ddl2Val.length);
		    data.addRows(json.assembliesNames.length);
		    for(var i=0; i<ddl2Val.length; i++){
			var tooltip = json.assembliesNames[i]+'\n'+ddl2Val[i]+','+ddl3Val[i];
			data.setCell(i,i, {ddl2Val[i], ddl3Val[i]});
			data.setCell(i,json.assembliesNames.length,tooltip);
		    }	
		    alert("options");
		    // Set chart options
		    var options = {'title':'Scatterplot of '+document.getElementById("ddl2").value+' AND '+document.getElementById("ddl3").value,
				   ...
				   'crosshair': {trigger: 'both' },   // Display crosshairs.'legend': 'none',
				   'width':700,
				   'height':600};
		    // Instantiate and draw our chart, passing in some options.
