...
if __name__ == '__main__':
    ...
    report = open(history[0]+'/report.txt', 'r').readlines()
    minThreshold = report[3].split()[3]
    maxThreshold = report[4].split()[3]
    ...
    for h in range(1,len(history)):
        if os.path.exists(history[h]+'/Combined_history'):
            ...
            history[h] += '/Scaffold'
        historyNr = h+1
        #Check if threshold values are the same
        report = open(history[h]+'/report.txt', 'r').readlines()
        if ((minThreshold != report[3].split()[3]) or (maxThreshold != report[4].split()[3])):
            sys.stderr.write('Input 1 with thresholds [' + str(minThreshold) + ',' + str(maxThreshold) + '] and input ' \
                + str(historyNr) + ' with thresholds [' + str(report[3].split()[3]) + ',' + str(report[4].split()[3]) \
                + '] cannot be compared')
