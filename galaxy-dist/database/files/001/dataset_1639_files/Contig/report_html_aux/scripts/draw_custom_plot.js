	...
	for(var y=1; y<json.report[x].length; y++){
		...
		if(json.report[x][y][z].metricName == document.getElementById("ddl1").value){
			...
			if (json.report[x][y][z].values[i].length != 0){
			    var len = json.report[x][y][z].values[i].length;
			    var value = json.report[x][y][z].values[i]
			    for(var g=0; g<len; g++){
				if(value[g].toString().indexOf('+') > -1){
				    value[g] = value[g].split(' + ')[0];
				}
			    }
			    data.addRow([json.assembliesNames[i], value[i]);//json.report[x][y][z].values[i]]);
			}
		    }
		    // Set chart options
